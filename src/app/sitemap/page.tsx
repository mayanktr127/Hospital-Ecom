      ...
      <main className="wrap max-w-[1040px] w-full mx-auto px-4 md:px-6 flex-1 py-10">
        ...
        <div className="rounded-[32px] bg-[#003865] text-white p-8 md:p-12 mb-10 shadow-xl relative overflow-hidden">
          ...
          <span className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-white/15 text-white text-xs font-semibold font-archivo mb-3 border border-white/20">
            <Map className="w-3.5 h-3.5 text-[#007AC1]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 font-inter text-[#003865]">
          {sections.map((sec) => {
            const Icon = sec.icon;
            return (
              <div
                key={sec.title}
                className="bg-white border border-[#003865]/12 rounded-[28px] p-6 shadow-sm flex flex-col justify-between"
              >
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <h2 className="font-archivo font-bold text-lg text-[#003865]">{sec.title}</h2>
                  </div>
                  ...
                  <ul className="space-y-2.5 list-none p-0 m-0 text-xs md:text-sm">
                    {sec.links.map((link) => (
                      <li key={link.label}>
                        <Link
                          href={link.href}
                          className="flex items-center justify-between text-[#4A607A] hover:text-[#007AC1] py-1 transition-colors group"
                        >
                          <span>{link.label}</span>
                          <ChevronRight className="w-4 h-4 text-[#007AC1] group-hover:translate-x-1 transition-transform" />
                        </Link>
                      </li>
                    ))}
                  </ul>
